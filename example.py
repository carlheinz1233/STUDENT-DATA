...
ckc1=PhotoImage(file='icon/ckc.png')
logo=PhotoImage(file='icon/logo.png')
settings=PhotoImage(file='icon/settings.png')
profile=PhotoImage(file='icon/profile.png')
home=PhotoImage(file='icon/home.png')
searchlogo=PhotoImage(file='icon/searchlogo.png')


# Initialize admin_data as an empty list 
admin_data = []


#Checking initial database
def init_database():
    if os.path.exists('students_account_lists.db'):
        
        connection=sqlite3.connect('students_account_lists.db')
...
def add_data(Username, Password, Name, Age, Date_of_Birth, Address, Contact_No, Guardians_Name, Guardians_Contact_No, Elementary, High_School, Senior_High_School, College, Semester, Subjects_Name, Subjects_Description, Credits_Units):
    connection = sqlite3.connect('students_account_lists.db')
    connection1 = sqlite3.connect('admin_data_lists.db')

    cursor = connection.cursor()  # Commit the changes
    cursor1 = connection1.cursor()  # Close the connection

    cursor.execute(f"""
    INSERT INTO data VALUES('{Username}', '{Password}', '{Name}', '{Age}', '{Date_of_Birth}', '{Address}', '{Contact_No}', '{Guardians_Name}', '{Guardians_Contact_No}', '{Elementary}', '{High_School}', '{Senior_High_School}', '{College}', '{Semester}', '{Subjects_Name}', '{Subjects_Description}', '{Credits_Units}')
    """)
...
#Adding/Inserting of data
def add_admin_data(admin_data):
    connection = sqlite3.connect('admin_data_lists.db')
    connection1 = sqlite3.connect('students_account_lists.db')
    ...
    cursor6 = connection1.cursor()

    # Fetch the data from the admin_data
    admin_data1 = []
    for row_fields in admin_data:
        row_data = []
        for field in row_fields:
            row_data.append(field)
        admin_data1.append(row_data)

    # Insert the data into the database
    for row in admin_data1:
        cursor.execute("INSERT INTO admin_data VALUES (?, ?, ?, ?, ?, ?)", row)
    
    # Insert the data into the database
    for row in admin_data1:
        cursor2.execute("INSERT INTO 'First Sem' VALUES (?, ?, ?, ?, ?, ?)", row)
    
    # Insert the data into the database
    for row in admin_data1:
        cursor3.execute("INSERT INTO 'Second Sem' VALUES (?, ?, ?, ?, ?, ?)", row)

    # Insert the data into the database
    for row in admin_data1:
        cursor4.execute("INSERT INTO admin_data VALUES (?, ?, ?, ?, ?, ?)", row)

    # Insert the data into the database
    for row in admin_data1:
        cursor5.execute("INSERT INTO 'First Sem' VALUES (?, ?, ?, ?, ?, ?)", row)
    
    # Insert the data into the database
    for row in admin_data1:
        cursor6.execute("INSERT INTO 'Second Sem' VALUES (?, ?, ?, ?, ?, ?)", row)
...
def Login():
    ...
    #Password
    password_ent=Entry(Student_Data2_fm,font=('Franklin Gothic Demi (Headings)',18),justify=CENTER, show="*")
...
def Sign_up():
    ...
    #Binding
    frame_box4.bind('<KeyRelease>', lambda e: remove_highlight_warning(entry=frame_box4))
    ...
    #Cancel button
    cancel_btn=Button(Student_Data4_fm, image=cancel1_btn,bd=0,bg='#323233',command=forward_to_Student_Data)
    cancel_btn.place(x=880,y=595,width=110)

    #Submit button
    submit_btn=Button(Student_Data4_fm, image=submit1_btn,bd=0,bg='#323233',command=check_input_validation)
    submit_btn.place(x=1020,y=595,width=110)
...
def Admin1():
    ...
    #1st year button
    firstyear_btn=Button(Student_Data6_fm,text='FIRST YEAR',bg='#1e5fe4',fg='white',font=('Franklin Gothic Demi (Headings)',18,'bold'),bd=0,relief=RAISED,command=forward_to_Admin2)
    firstyear_btn.place(x=450,y=220,width=200)

    #2nd year button
    secondyear_btn=Button(Student_Data6_fm,text='SECOND YEAR',bg='#1e5fe4',fg='white',font=('Franklin Gothic Demi (Headings)',18,'bold'),bd=0,relief=RAISED,command=forward_to_Admin2)
    secondyear_btn.place(x=450,y=300,width=200)

    #3rd year button
    thirdyear_btn=Button(Student_Data6_fm,text='THIRD YEAR',bg='#1e5fe4',fg='white',font=('Franklin Gothic Demi (Headings)',18,'bold'),bd=0,relief=RAISED,command=forward_to_Admin2)
    thirdyear_btn.place(x=450,y=380,width=200)

    #4th year button
    fourthyear_btn=Button(Student_Data6_fm,text='FOURTH YEAR',bg='#1e5fe4',fg='white',font=('Franklin Gothic Demi (Headings)',18,'bold'),bd=0,relief=RAISED,command=forward_to_Admin2)
    fourthyear_btn.place(x=450,y=460,width=200)
...
def Admin2():
    ...
    #Forwarding to Student Data and destroying the content of Admin2
    def forward_to_Student_Data():
        Student_Data7_fm.destroy()
        Student_Data()

    #Forwarding to view_student_info and destroying the content of Admin2
    def forward_to_view_student_info():
        Student_Data7_fm.destroy()
        view_student_info()

    #Forwarding to edit_student_info and destroying the content of Admin2
    def forward_to_edit_student_info():
        Student_Data7_fm.destroy()
        edit_student_info()

    #Forwarding to print_student_info and destroying the content of Admin2
    ...
    firstyear_btn=Button(Student_Data7_fm,text="VIEW STUDENT'S INFO",bg='#1e5fe4',fg='white',font=('Franklin Gothic Demi (Headings)',18,'bold'),bd=0,relief=RAISED,command=forward_to_view_student_info)
    firstyear_btn.place(x=400,y=220,width=300)

    #Edit students info button
    secondyear_btn=Button(Student_Data7_fm,text="EDIT STUDENT'S INFO",bg='#1e5fe4',fg='white',font=('Franklin Gothic Demi (Headings)',18,'bold'),bd=0,relief=RAISED,command=forward_to_edit_student_info)
    secondyear_btn.place(x=400,y=300,width=300)

    #Print students info button
    thirdyear_btn=Button(Student_Data7_fm,text="PRINT STUDENT'S INFO",bg='#1e5fe4',fg='white',font=('Franklin Gothic Demi (Headings)',18,'bold'),bd=0,relief=RAISED,command=forward_to_print_student_info)
...
def first_year_user():
    ...
    #Forwarding to Login1 and destroying the content of first_year_user
    def forward_Login1():
        Student_Data8_fm.destroy()
        Login1()

    #Set values in combobox
    semester_list = ['Semester','1st Sem','2nd Sem']
    ...
    #Heading of Settings
    settinglogo_btn=Button(Student_Data8_fm,image=settings,bg='#323233',fg='white',font=('Franklin Gothic Demi (Headings)',13,'bold'),bd=0,relief=RAISED)
    settinglogo_btn.place(x=1365,y=10)

    #Heading of Profile
    profilelogo_btn=Button(Student_Data8_fm,image=profile,bg='#323233',fg='white',font=('Franklin Gothic Demi (Headings)',13,'bold'),bd=0,relief=RAISED)
    profilelogo_btn.place(x=1420,y=5)

    #Heading of Home
    profilelogo_btn=Button(Student_Data8_fm,image=home,bg='#323233',fg='white',font=('Franklin Gothic Demi (Headings)',13,'bold'),bd=0,relief=RAISED,command=forward_Login1)
    ...
    # Connect to the database
        ...
        if user_data:
            # Fetch the subject data for the logged-in user from the 'First Sem' table
            cursor.execute("SELECT SubjectName, SubjectDescription, CreditsUnits, Grades, Remarks FROM 'First Sem' WHERE Name = ?", (user_data[2],))
            ...
            row_text = "High School: {}\nSenior High School: {}\nCollege: {}\nSemester: {}\nSubjects Name: {}\nSubjects Description: {}\nCredits Units: {}".format(
                user_data[10], user_data[11], user_data[12], user_data[13], user_data[14], user_data[15], user_data[16]
            )
    ...
    def view_entries(event):
        # Connect to the database
        conn = sqlite3.connect('admin_data_lists.db')
        cursor = conn.cursor()
    ...
    # Save button
    save_btn = Button(Student_Data8_fm, text="Save", font=('Franklin Gothic Demi (Headings)', 16, 'bold'), bg='#6aa84f', fg='white', relief=RAISED)
...
def view_student_info():
    #Forwarding to Admin1 and destroying the contents of view_student_info
    ...
    #Frame of its container
    Student_Data9_fm=Frame(Student_Data_fm, background=bg_color, highlightbackground=bg_color, highlightthickness=3)
    ...
    #Heading of Home
    profilelogo_btn=Button(Student_Data9_fm,image=home,bg='#323233',fg='white',font=('Franklin Gothic Demi (Headings)',13,'bold'),bd=0,relief=RAISED,command=forward_to_Admin1)
    profilelogo_btn.place(x=1350,y=5)

    #Heading of Name Entry(User's Name)
    name_entry_ent = Entry(Student_Data9_fm, highlightbackground=bg_color, highlightthickness=3,bd=1,font=('Franklin Gothic Demi (Headings)',15,'bold'))
    name_entry_ent.place(x=100,y=105,width=190,height=35)

    #himoon an funtion na clear entries pa view entries tapos insert imbis na delete, katapos kay makadto 1st year(himoon and clear entries into view entries)
    def display_student_data(name='ADMIN'):
        ...
        if data:
            ...
            for row in data:
                ...
                data_label = Label(Student_Data9_fm, font=('Franklin Gothic Demi (Headings)', 18), wraplength=800, justify='left', bg='#323233',width=40,height=8)
                ...
                #Other label that was displayed at the right
                data_label1 = Label(Student_Data9_fm, font=('Franklin Gothic Demi (Headings)', 18), wraplength=800, justify='left', bg='#323233',width=50,height=9)
                ...
                data_label1.config(text=row_text, fg='white')
                 ...
                 # Fetch the subject data for the given name
                cursor.execute("SELECT SubjectName, SubjectDescription, CreditsUnits, Grades, Remarks FROM data WHERE Name = ?", (name,))
                subject_data = cursor.fetchall()

                # Display the subject data in the table
                for i, subject in enumerate(subject_data, start=1):
                    for j, field in enumerate(subject):
                        input_fields[i-1][j].insert(0, field)
        else:
            # No data found
            text=Text(Student_Data9_fm,wrap=WORD,font=('Franklin Gothic Demi (Headings)', 18), fg='white', bg='#323233',width=91,height=8)
    ...
    #Search button logo
    search_btn = Button(Student_Data9_fm, image=searchlogo, highlightthickness=3, relief=RAISED, command=search_user)
    search_btn.place(x=290,y=108,height=30)

    # Save button
    save_btn = Button(Student_Data9_fm, text="Save", font=('Franklin Gothic Demi (Headings)', 16, 'bold'), bg='#6aa84f', fg='white', relief=RAISED)
    save_btn.place(x=1420, y=690)
...
def edit_student_info():
    #Forwarding to Admin1 and destroying the contents of edit_student_info
    def forward_to_Admin1():
    ...
    #Frame of its container
    Student_Data10_fm=Frame(Student_Data_fm, background=bg_color, highlightbackground=bg_color, highlightthickness=3)
    ...
    #Heading of Home
    profilelogo_btn=Button(Student_Data10_fm,image=home,bg='#323233',fg='white',font=('Franklin Gothic Demi (Headings)',13,'bold'),bd=0,relief=RAISED,command=forward_to_Admin1)
    profilelogo_btn.place(x=1350,y=5)

    #Heading of Name Entry(User's Name)
    name_entry1_ent = Entry(Student_Data10_fm, highlightbackground=bg_color, highlightthickness=3,bd=1,font=('Franklin Gothic Demi (Headings)',15,'bold'))
    ...
    def display_student_data(name='ADMIN'):
        ...
        if data:
            ...
            for row in data:
                ...
                data_label = Label(Student_Data10_fm, font=('Franklin Gothic Demi (Headings)', 18), wraplength=800, justify='left', bg='#323233',width=40,height=8)
                data_label.place(x=320, y=100)
                ...
                #Other label that was displayed at the right
                data_label1 = Label(Student_Data10_fm, font=('Franklin Gothic Demi (Headings)', 18), wraplength=800, justify='left', bg='#323233',width=50,height=9)
                data_label1.place(x=840, y=80)
            ...
            # No data found
            text=Text(Student_Data10_fm,wrap=WORD,font=('Franklin Gothic Demi (Headings)', 18), fg='white', bg='#323233',width=91,height=8)
            text.insert(INSERT,"No data found.")
            text.place(x=350,y=100)
    ...
    #Search button logo
    search1_btn = Button(Student_Data10_fm, image=searchlogo, highlightthickness=3, relief=RAISED, command=search_user)
    ...
    #Saving view students info's data 
    ...
    def save_table_data():
        # Connect to the database
        conn = sqlite3.connect('admin_data_lists.db')
        conn1 = sqlite3.connect('students_account_lists.db')
        # Connect to the database
        cursor = conn.cursor()
        ...
        # Fetch the student's data from the database based on the name
        cursor.execute("SELECT * FROM data WHERE Name = ?", (Name,))
        student_data = cursor.fetchone()

        if student_data:
            # Iterate over the input fields and update the corresponding row in the database
            for i, row_fields in enumerate(input_fields):
                ...
                # Insert the data into the admin_data_lists.db database
                cursor.execute("""
                INSERT INTO 'First Sem' (SubjectName, SubjectDescription, CreditsUnits, Grades, Remarks, Name)
                VALUES (?, ?, ?, ?, ?, ?)
                """, (SubjectName, SubjectDescription, CreditsUnits, Grades, Remarks, Name))

                # Insert the data into the students_account_lists.db database
                cursor1.execute("""
                INSERT INTO 'First Sem' (SubjectName, SubjectDescription, CreditsUnits, Grades, Remarks, Name)
                VALUES (?, ?, ?, ?, ?, ?)
                """, (SubjectName, SubjectDescription, CreditsUnits, Grades, Remarks, Name))
                ...
                cursor.execute("""
                INSERT INTO 'Second Sem' (SubjectName, SubjectDescription, CreditsUnits, Grades, Remarks, Name)
                VALUES (?, ?, ?, ?, ?, ?)
                """, (SubjectName, SubjectDescription, CreditsUnits, Grades, Remarks, Name))
            ...
            # No student data found for the given name
            print("No student data found for the name:", Name)
    ...
    print(admin_data)
    add_admin_data(admin_data)  # Calling add_admin_data
    display_student_data()  # Update the display after saving the data

    # Save button
    save_btn = Button(Student_Data10_fm, text="Save", font=('Franklin Gothic Demi (Headings)', 16, 'bold'), bg='#6aa84f', fg='white', relief=RAISED, command=save_table_data)
    save_btn.place(x=1420, y=690)
...
def second_year_user():
   ...
   #Forwarding to Login1 and destroying the content of second_year_user
    def forward_Login1():
    ...
    #Frame of its container
    Student_Data11_fm=Frame(Student_Data_fm, bg=bg_color, highlightbackground=bg_color, highlightthickness=3)

    #Heading of CKC
    heading1_lb=Label(Student_Data11_fm,text='Christ the King College',image=ckc1,compound=LEFT,bg='#323233',fg='white',font=('Franklin Gothic Demi (Headings)',25,'bold'))
    heading1_lb.pack(side=TOP)

    #Heading of Student Data Logo
    studentlogo1_lb=Label(Student_Data11_fm,image=logo,compound=LEFT,bg='#323233',fg='white',font=('Franklin Gothic Demi (Headings)',13,'bold'))
    studentlogo1_lb.place(x=0,y=0,height=98)
    
    #Heading of Settings
    settinglogo1_btn=Button(Student_Data11_fm,image=settings,bg='#323233',fg='white',font=('Franklin Gothic Demi (Headings)',13,'bold'),bd=0,relief=RAISED)
    settinglogo1_btn.place(x=1365,y=10)

    #Heading of Profile
    profilelogo1_btn=Button(Student_Data11_fm,image=profile,bg='#323233',fg='white',font=('Franklin Gothic Demi (Headings)',13,'bold'),bd=0,relief=RAISED)
    profilelogo1_btn.place(x=1420,y=5)

    #Heading of Home
    profilelogo1_btn=Button(Student_Data11_fm,image=home,bg='#323233',fg='white',font=('Franklin Gothic Demi (Headings)',13,'bold'),bd=0,relief=RAISED,command=forward_Login1)
    profilelogo1_btn.place(x=1270,y=5)
    ...
    # Connect to the database
        ...
        if user_data:
            ...
            # Create labels to display the user data
            data_label = Label(Student_Data11_fm, font=('Franklin Gothic Demi (Headings)', 18), wraplength=800, justify='left', bg='#323233')
            data_label.place(x=350, y=100)
            ...
            # Other label that was displayed at the right
            data_label1 = Label(Student_Data11_fm, font=('Franklin Gothic Demi (Headings)', 18), wraplength=800, justify='left', bg='#323233')
            data_label1.place(x=890, y=100)
            ...
            # No data found for the logged-in user
            data_label = Label(Student_Data11_fm, text="No data found for the logged-in user.", font=('Franklin Gothic Demi (Headings)', 18), fg='white', bg='#323233')
            data_label.place(x=350, y=100)
    ...
    # Create the table
    table1_frame = Frame(Student_Data11_fm, bg='#323233')
    table1_frame.place(x=100, y=360)

    # Create the table headers
    headers = ['Subject Name', 'Subject Description', 'Credits/Units', 'Grades', 'Remarks']  
    for i, header in enumerate(headers):
        header_label = Label(table1_frame, text=header, font=('Franklin Gothic Demi (Headings)', 16, 'bold'), fg='white', bg='#323233')
        header_label.grid(row=0, column=i, padx=20, pady=10)
    ...
    for i in range(1, 8):
        ...
        for j in range(5):
            if j == 2 or j == 3:  # Credits/Units and Grades fields
                entry = Entry(table1_frame, font=('Franklin Gothic Demi (Headings)', 14), justify=CENTER)
            else:
                entry = Entry(table1_frame, font=('Franklin Gothic Demi (Headings)', 14))
            entry.grid(row=i, column=j, padx=20, pady=10)
            row_fields.append(entry)
        input_fields.append(row_fields)
    ...
    def view_entries(event):
        ...
        selected_option = select_sem1_btn.get()
        if selected_option == '1st Sem':
            for row_fields in enumerate (input_fields):
                for field in enumerate (data[i]):
                    row_fields[j].delete(0, 'end')
                    row_fields[j].insert(0, field)
                conn.close()
    ...
    # Create combobox widget for semester list
    select_sem1_btn = ttk.Combobox(Student_Data11_fm,font=('Franklin Gothic Demi (Headings)', 12, 'bold'),state='readonly',values=semester_list)
    select_sem1_btn.place(x=8,y=420,width=100)

    # Set the initial selection to the placeholder value
    select_sem1_btn.current(0)

    # Bind the clear_entries function to the <<ComboboxSelected>> event
    select_sem1_btn.bind('<<ComboboxSelected>>', view_entries)

    # Save button
    save_btn = Button(Student_Data11_fm, text="Save", font=('Franklin Gothic Demi (Headings)', 16, 'bold'), bg='#6aa84f', fg='white', relief=RAISED)
    save_btn.place(x=1420, y=690)
...
def third_year_user():
    ...
    #Forwardin to Login1 and destroying the content of third_year_user 
    def forward_Login1():
    ...
    #Heading of CKC
    heading2_lb=Label(Student_Data12_fm,text='Christ the King College',image=ckc1,compound=LEFT,bg='#323233',fg='white',font=('Franklin Gothic Demi(Headings)',25,'bold'))
    heading2_lb.pack(side=TOP)

    #Heading of Student Data
    studentlogo2_lb=Label(Student_Data12_fm,image=logo,compound=LEFT,bg='#323233',fg='white',font=('Franklin Gothic Demi(Heading)',13,'bold'),bd=0,relief=RAISED)
    studentlogo2_lb.place(x=0,y=0,height=98)

    #Heading of Settings
    settingslogo2_btn=Button(Student_Data12_fm,image=settings,bg='#323233',fg='white',font=('Franklin Gothic Demi (Headings)',13,'bold'),bd=0,relief=RAISED)
    settingslogo2_btn.place(x=1365,y=10)

    #Headings of Profile
    profilelogo2_btn=Button(Student_Data12_fm,image=profile,bg='#323233',fg='#323233',font=('Franklin Gothic Demi (Headings)',13,'bold'),bd=0,relief=RAISED)
    profilelogo2_btn.place(x=1420,y=5)

    #Heading of Home
    profilelogo2_btn=Button(Student_Data12_fm,image=home,bg='#323233',fg='white',font=('Franklin Gothic Demi (Headings)',13,'bold'),bd=0,relief=RAISED,command=forward_Login1)
    profilelogo2_btn.place(x=1270,y=5)
    ...
    # Connect to the database
        ...
        if user_data:
            ...
            # Create labels to display the user data
            data_label = Label(Student_Data12_fm, font=('Franklin Gothic Demi (Headings)', 18), wraplength=800, justify='left', bg='#323233')
            data_label.place(x=350, y=100)
            ...
            # Other label that was displayed at the right
            data_label1 = Label(Student_Data12_fm, font=('Franklin Gothic Demi (Headings)', 18), wraplength=800, justify='left', bg='#323233')
            data_label1.place(x=890, y=100)
            ...
            # No data found for the logged-in user
            data_label = Label(Student_Data12_fm, text="No data found for the logged-in user.", font=('Franklin Gothic Demi (Headings)', 18), fg='white', bg='#323233')
            data_label.place(x=350, y=100)
    ...
    # Create the table
    table2_frame = Frame(Student_Data12_fm,bg='#323233')
    table2_frame.place(x=100, y=360)

    # Create the table headers
    headers = ['Subject Name', 'Subject Description', 'Credits/Units', 'Grades', 'Remarks']  
    for i, header in enumerate(headers):
        header_label = Label(table2_frame, text=header, font=('Franklin Gothic Demi (Headings)', 16, 'bold'), fg='white', bg='#323233')
        header_label.grid(row=0, column=i, padx=20, pady=10)
    ...
    for i in range(1, 8):
        ...
        for j in range(5):
            if j == 2 or j == 3:  # Credits/Units and Grades fields
                entry = Entry(table2_frame, font=('Franklin Gothic Demi (Headings)', 14), justify=CENTER)
            else:
                entry = Entry(table2_frame, font=('Franklin Gothic Demi (Headings)', 14))
            entry.grid(row=i, column=j, padx=20, pady=10)
            row_fields.append(entry)
        input_fields.append(row_fields)

    def view_entries(event):
        selected_option = select_sem2_btn.get()
        if selected_option == '1st Sem':
            for row_fields in input_fields:
                for field in row_fields:
                    field.insert(0, 'end')
    ...
    # Create combobox widget for semester list
    select_sem2_btn = ttk.Combobox(Student_Data12_fm,font=('Franklin Gothic Demi (Headings)', 12, 'bold'),state='readonly',values=semester_list)
    select_sem2_btn.place(x=8,y=420,width=100)

    # Set the initial selection to the placeholder value
    select_sem2_btn.current(0)
    
    # Bind the clear_entries function to the <<ComboboxSelected>> event
    select_sem2_btn.bind('<<ComboboxSelected>>', view_entries)

    # Save button
    save2_btn = Button(Student_Data12_fm,text="Save", font=('Franklin Gothic Demi (Headings)',16,'bold'),bg='#6aa84f',fg='white', relief=RAISED)
    save2_btn.place(x=1420,y=690)
...
def fourth_year_user():
    ...
    #forwarding to Login1 and destroying the content of fourth_year_user
    def forward_Login1():
    ...
    #Frame of its container
    Student_Data13_fm=Frame(Student_Data_fm, bg=bg_color, highlightbackground=bg_color, highlightthickness=3)

    #Heading of CKC
    heading3_1b=Label(Student_Data13_fm,text='Christ The King College',image=ckc1,compound=LEFT,bg='#323233',fg='white',font=('Franklin Gothic Demi (headings)',25,'bold'))
    heading3_1b.pack(side=TOP)

    #Heading of Student Data Logo
    studentlogo3_lb=Label(Student_Data13_fm,image=logo,compound=LEFT,bg='#323233',fg='white',font=('Franklin Gothic Demi (Headings)',13,'bold'))
    studentlogo3_lb.place(x=0,y=0,height=98)
    
    #Heading of Settings
    settinglogo3_btn=Button(Student_Data13_fm,image=settings,bg='#323233',fg='white',font=('Franklin Gothic Demi (Headings)',13,'bold'),bd=0,relief=RAISED)
    settinglogo3_btn.place(x=1365,y=10)

    #Heading of Profile
    profilelogo3_btn=Button(Student_Data13_fm,image=profile,bg='#323233',fg='white',font=('Franklin Gothic Demi (Headings)',13,'bold'),bd=0,relief=RAISED)
    profilelogo3_btn.place(x=1420,y=5)

    #Heading of Home
    profilelogo3_btn=Button(Student_Data13_fm,image=home,bg='#323233',fg='white',font=('Franklin Gothic Demi (Headings)',13,'bold'),bd=0,relief=RAISED,command=forward_Login1)
    profilelogo3_btn.place(x=1270,y=5)
    ...
    # Connect to the database
        ...
        if user_data:
            ...
            # Create labels to display the user data
            data_label = Label(Student_Data13_fm, font=('Franklin Gothic Demi (Headings)', 18), wraplength=800, justify='left', bg='#323233')
            data_label.place(x=350, y=100)
            ...
            # Other label that was displayed at the right
            data_label1 = Label(Student_Data13_fm, font=('Franklin Gothic Demi (Headings)', 18), wraplength=800, justify='left', bg='#323233')
            data_label1.place(x=890, y=100)
            ...
            # No data found for the logged-in user
            data_label = Label(Student_Data13_fm, text="No data found for the logged-in user.", font=('Franklin Gothic Demi (Headings)', 18), fg='white', bg='#323233')
            data_label.place(x=350, y=100)
    ...
    # Create the table
    table3_frame = Frame(Student_Data13_fm, bg='#323233')
    table3_frame.place(x=100, y=360)

    # Create the table headers
    headers = ['Subject Name', 'Subject Description', 'Credits/Units', 'Grades', 'Remarks']  
    for i, header in enumerate(headers):
        header_label = Label(table3_frame, text=header, font=('Franklin Gothic Demi (Headings)', 16, 'bold'), fg='white', bg='#323233')
        header_label.grid(row=0, column=i, padx=20, pady=10)
    ...
    for i in range(1, 8):
        ...
        for j in range(5):
            if j == 2 or j == 3:  # Credits/Units and Grades fields
                entry = Entry(table3_frame, font=('Franklin Gothic Demi (Headings)', 14), justify=CENTER)
            else:
                entry = Entry(table3_frame, font=('Franklin Gothic Demi (Headings)', 14))
            entry.grid(row=i, column=j, padx=20, pady=10)
            row_fields.append(entry)
        input_fields.append(row_fields)

    def view_entries(event):
        selected_option = select_sem3_btn.get()
        if selected_option == '1st Sem':
            for row_fields in input_fields:
                for field in row_fields:
                    field.insert(0, 'end')
    ...
    # Create combobox widget for semester list
    select_sem3_btn = ttk.Combobox(Student_Data13_fm,font=('Franklin Gothic Demi (Headings)', 12, 'bold'),state='readonly',values=semester_list)
    select_sem3_btn.place(x=8,y=420,width=100)

    # Set the initial selection to the placeholder value
    select_sem3_btn.current(0)

     # Bind the clear_entries function to the <<ComboboxSelected>> event
    select_sem3_btn.bind('<<ComboboxSelected>>', view_entries)

    # Save button
    save3_btn = Button(Student_Data13_fm, text="Save", font=('Franklin Gothic Demi (Headings)', 16, 'bold'), bg='#6aa84f', fg='white', relief=RAISED)
    save3_btn.place(x=1420, y=690)
...
def print_student_info():
    ...
    #Frame of its container
    Student_Data14_fm=Frame(Student_Data_fm, background=bg_color, highlightbackground=bg_color, highlightthickness=3)

    #Heading of CKC
    heading_lb=Label(Student_Data14_fm,text='Christ the King College',image=ckc1,compound=LEFT,bg='#323233',fg='white',font=('Franklin Gothic Demi (Headings)',25,'bold'))
    heading_lb.pack(side=TOP)

    #Heading of Student Data Logo
    studentlogo_lb=Label(Student_Data14_fm,image=logo,compound=LEFT,bg='#323233',fg='white',font=('Franklin Gothic Demi (Headings)',13,'bold'))
    studentlogo_lb.place(x=0,y=0,height=98)
    
    #Heading of Home
    profilelogo_btn=Button(Student_Data14_fm,image=home,bg='#323233',fg='white',font=('Franklin Gothic Demi (Headings)',13,'bold'),bd=0,relief=RAISED,command=forward_to_Admin1)
    profilelogo_btn.place(x=1350,y=5)

    #Heading of Name Entry(User's Name)
    name_entry1_ent = Entry(Student_Data14_fm,highlightbackground=bg_color,highlightthickness=3,bd=1,font=('Franklin Gothic Demi (Headings)',15,'bold'))
    name_entry1_ent.place(x=100,y=105,width=190,height=35)

    def display_student_data(name='ADMIN'):
        # Clear any existing data labels
        for widget in Student_Data14_fm.winfo_children():
            if isinstance(widget, Label) and widget.winfo_x() in (350, 890):
                widget.destroy()
        ...
        if data:
            ...
            for row in data:
                ...
                data_label = Label(Student_Data14_fm, font=('Franklin Gothic Demi (Headings)', 18), wraplength=800, justify='left', bg='#323233',width=40,height=8)
                data_label.place(x=320, y=100)
                ...
                #Other label that was displayed at the right
                data_label1 = Label(Student_Data14_fm, font=('Franklin Gothic Demi (Headings)', 18), wraplength=800, justify='left', bg='#323233',width=50,height=9)
                data_label1.place(x=840, y=80)
                ...
                cursor.execute("SELECT SubjectName, SubjectDescription, CreditsUnits, Grades, Remarks FROM 'First Sem' WHERE Name = ?", (name,))
            ...
            # No data found
            text=Text(Student_Data14_fm,wrap=WORD,font=('Franklin Gothic Demi (Headings)', 18), fg='white', bg='#323233',width=91,height=8)
            text.insert(INSERT,"No data found.")
            text.place(x=350,y=100)
    ...
    # Create the table
    table_frame = Frame(Student_Data14_fm, bg='#323233')
    table_frame.place(x=100, y=360)
    ...
    #Search button logo
    search1_btn = Button(Student_Data14_fm, image=searchlogo, highlightthickness=3, relief=RAISED, command=search_user)
    search1_btn.place(x=290,y=108,height=30)

    # Create combobox widget for semester list
    select_sem_btn = ttk.Combobox(Student_Data14_fm,font=('Franklin Gothic Demi (Headings)', 12, 'bold'),state='readonly',values=semester_list)
    select_sem_btn.place(x=8,y=420,width=100)
    ...
    def print_data(name):
        ...
        # Fetch the data from the database for the specified name
        cursor.execute("SELECT * FROM 'First Sem' WHERE Name = ?", (name,))
        first_sem_data = cursor.fetchall()

        cursor.execute("SELECT * FROM 'Second Sem' WHERE Name = ?", (name,))
        ...
        if file_name:
            ...
            with open(file_name, 'w') as file:
                ...
                for data in first_sem_data:
                    file.write(str(data) + '\n')

                file.write('\n\n')

                for data in second_sem_data:
                    file.write(str(data) + '\n')

            # Open the saved file with the default text editor
    ...
    #Button of Print
    print_btn = Button(Student_Data14_fm, text="Print", font=('Franklin Gothic Demi (Headings)', 16, 'bold'), bg='#6aa84f', fg='white', relief=RAISED, command=lambda: search_user())
    print_btn.place(x=1420, y=635, width=70)
    ...
    # Save button
    save_btn = Button(Student_Data14_fm, text="Save", font=('Franklin Gothic Demi (Headings)', 16, 'bold'), bg='#6aa84f', fg='white', relief=RAISED, command=save_table_data)
    save_btn.place(x=1420, y=690)

    # Frame of its container
    Student_Data14_fm.pack(fill='both', expand=1)
    Student_Data14_fm.pack_propagate(False)
    display_student_data()
